...
def GEV_Parameters(df: pd.DataFrame, GEV_Parameters: np.ndarray, bounds, ID: str, PMP: float)->pd.DataFrame:
    ''' Function defines an objective function for finding the GEV parameters and
    then determines the best GEV parameters that minimize the differense between the
    GEV and comparison data.
    '''    
...
def GEV_parameters_Fit(raw_precip: pd.DataFrame, ID: str, PMP: float)->pd.DataFrame:
    '''This function provides initial value for finding the GEV parameters and then
    finds the best GEV parameters using the function GEV_parameters.
    '''
...
def Avg_R(lower_bound: float, upper_bound: float, GEV_parameters: np.ndarray, PMP: float)-> float:
    '''Calculates the average value of the GEV distribution of rainfall based on 
    an uppe rand lower bound.
    '''
...
def PDF_QlS(Q: float, S: float, mu: float, GEV_parameters: np.ndarray, PMP: float)-> float:
    '''This function provides the runoff PDF conditional on Max. Potential Retention, where
...
def runoff_RI(RI: float, tck_RI_Q)-> float:
...
def S_avg_integrand(S: float, alpha: float, beta: float, S_limit: float)-> float:
    '''Defines the integrand for finding the average value over each patition
    '''
...
def S_avg_partition(alpha: float, beta: float, S_limit: float, lower_bound: float, upper_bound: float)-> float:
    '''Defines the integration for the average value over each patition
    '''
...
#####################################################
### Functions for calculating data for input 
### to the mean preicpitation curve calcuation
#####################################################
...
def plot_runoff_maxRetention_distributions(GEV_parameters_E: np.ndarray, PMP:float, fitted_cn: pd.DataFrame)-> plt.subplots:
    '''Plots the distribution of runoff conditional on the max. potential retention 
    and plots the distribution of the max. potential retention
    '''
