...
def Q_SCS(R: float, CN: float, mu: float) -> float:
...
def PPF_GEV(P: float, x: np.ndarray, PMP: float) -> float:
    """
    """
...
def Avg_R(lower_bound: float, upper_bound: float, GEV_parameters: np.ndarray, PMP: float) -> float:
    """Calculates the average value of the GEV distribution of rainfall based on an upper rand lower bound.
    """
...
def Bound_rainfall_L(RI_upper: float, RI_middle: float, GEV_parameters: np.ndarray, initial_value: np.ndarray, 
                     PMP: float) -> float:
...
def objective_func_bound_runoff_L(RI_lower: float, RI_upper: float, RI_middle: float, mu: float, 
    ...
    """ 
    return (np.square(Avg_Q(runoff_RI(RI_lower, tck_RI_Q), runoff_RI(RI_upper,  tck_RI_Q), mu, \
                            GEV_parameters, PMP, partition_avg, Delta_P)[0]/(1.0/RI_lower -1.0/RI_upper) \
                      - runoff_RI(RI_middle, tck_RI_Q)))
...
def weights_Rainfall(Return_Intervals: np.ndarray, GEV_parameters: np.ndarray, PMP: float, RI_upper_bound: float, 
    ...
    """RI_data is the return intervals from which values for the rainfall are taken directly from the input data 
       (NOAA_precip) instead of being calculated from the fitted GEV.
    """
...
#Provides the same output as the previous function.
...
def mean_curve_input_table(CL: np.ndarray, return_interval_data: pd.DataFrame, PMP: float, outputs_dir: WindowsPath, 
                           Project_Area: str, Pluvial_Model: str, BCN: str) -> pd.DataFrame:
    """This functions takes the return interval data and creates an input table of values for calculating the mean 
       curve. The function returns a table of precipitation value for the different AEP and confidence limits (CL) based 
       on a lognormal distribution that represents the variability of precipitation based on the 90-percent confidence 
       interval limits provided by NOAA Atlas 14.
    """
