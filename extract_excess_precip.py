...
def create_event_total_dataframe(reduced_event_totals: dict, unreduced_event_totals: dict, domain: str) -> pd.DataFrame:
    """Create a dataframe of the reduced and unreduced excess precipitation for the specified domain."""
    event_totals_df = pd.DataFrame.from_dict({"Reduced_Excess_P": reduced_event_totals[domain]})
    if domain in unreduced_event_totals.keys():
        ...
        if not all(unreduced_event_totals_df.index == event_totals_df.index):
            raise ValueError("The reduced and unreduced event IDs do not match, cannot concatenate the dataframes")
        event_totals_df = pd.concat([unreduced_event_totals_df, event_totals_df], axis=1)
        if not all(event_totals_df["Reduced_Excess_P"] <= event_totals_df["Unreduced_Excess_P"]):
            raise ValueError("One or more events have reduced excess precipitation that is greater than the unreduced.")
...
reduced_event_totals = extract_event_totals(forcing_files)
unreduced_event_totals = extract_event_totals(unreduced_forcing_files)

writer = pd.ExcelWriter(event_excess_path)
for domain in reduced_event_totals.keys():
    event_totals_df = create_event_total_dataframe(reduced_event_totals, unreduced_event_totals, domain)
